...
namespace AbieSoft\Application\Console\Database;
...
class Backup
{
    ...
    {
        $dname = Config::envReader('DBS_NAME');
    ...
    protected static function makeBackup($tabel, $j, $namabackupan)
    {
        ...
        $nama = $tabel;
        $namafolderBackup = "";
        for ($nf = 0; $nf < count(explode(" ", Config::envReader('APP_TITLE'))); $nf++) {
            $namafolderBackup .= strtolower(str_replace(".", "", explode(" ", Config::envReader('APP_TITLE'))[$nf]));
        }
        ...
        fwrite($file, $isi);
        $isi = "use AbieSoft\Application\Mysql\DB;\n";
        fwrite($file, $isi);
        $isi = "use AbieSoft\Application\Mysql\Schema;\n";
        fwrite($file, $isi);
        ...
        $cekkolom = DB::terhubung()->query("SELECT * FROM information_schema.columns WHERE table_schema = '" . Config::envReader('DBS_NAME') . "' AND table_name = '" . $tabel . "' ")->hasil();
